...
public class EntityUtil implements Util {
   ...
   public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
      ...
      while(true) {
         ...
         } while(esohack.friendManager.isFriend(player));
   ...
   public static float getDifficultyMultiplier(float distance) {
      switch(mc.field_71441_e.func_175659_aa()) {
      case HARD:
   ...
   public static int getEnchantmentModifierDamage(Iterable<ItemStack> stacks, DamageSource source) {
      ...
      while(var3.hasNext()) {
         ...
         for(int i = 0; i < nbttaglist.func_74745_c(); ++i) {
            ...
            if (enchantment != null && enchantment instanceof EnchantmentProtection) {
               modifier += enchantment.func_77318_a(nbttaglist.func_150305_b(i).func_74765_d("lvl"), source);
            }
         }
      }
   ...
   public static float calculate(double x, double y, double z, EntityLivingBase base) {
      double distance = base.func_70092_e(x, y, z) / 144.0D;
      if (distance > 1.0D) {
         return 0.0F;
      } else {
         double densityDistance = distance = (1.0D - distance) * (double)mc.field_71441_e.func_72842_a(new Vec3d(x, y, z), base.func_174813_aQ());
         float damage = getDifficultyMultiplier((float)((densityDistance * densityDistance + distance) / 2.0D * 7.0D * 12.0D + 1.0D));
         DamageSource damageSource = DamageSource.func_94539_a(new Explosion(mc.field_71441_e, mc.field_71439_g, x, y, z, 6.0F, false, true));
         ...
         int modifierDamage = getEnchantmentModifierDamage(base.func_184193_aE(), damageSource);
         if (modifierDamage > 0) {
   ...
   public static List<Vec3d> getUnsafeBlocksFromVec3d(Vec3d pos, int height, boolean floor, boolean face) {
      List<Vec3d> vec3ds = new ArrayList();
      Vec3d[] var5 = getOffsets(height, floor, face);
      int var6 = var5.length;

      for(int var7 = 0; var7 < var6; ++var7) {
         Vec3d vector = var5[var7];
         BlockPos targetPos = (new BlockPos(pos)).func_177963_a(vector.field_72450_a, vector.field_72448_b, vector.field_72449_c);
         Block block = mc.field_71441_e.func_180495_p(targetPos).func_177230_c();
         if (block instanceof BlockAir || block instanceof BlockLiquid || block instanceof BlockTallGrass || block instanceof BlockFire || block instanceof BlockDeadBush || block instanceof BlockSnow) {
            vec3ds.add(vector);
   ...
   public static List<Vec3d> getUntrappedBlocksExtended(int extension, EntityPlayer player, boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean raytrace, boolean noScaffoldExtend, boolean face) {
      ...
      if (extension == 1) {
         ...
         for(var12 = MathUtil.getBlockBlocks(player).iterator(); var12.hasNext(); ++extend) {
            vec3d = (Vec3d)var12.next();
            if (extend > extension) {
               break;
            }
   ...
   public static List<Vec3d> targets(Vec3d vec3d, boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean raytrace, boolean face) {
      ...
      if (antiStep) {
         ...
         if (vec3ds.size() == 4) {
            Iterator var10 = vec3ds.iterator();

            label34:
            while(var10.hasNext()) {
               Vec3d vector = (Vec3d)var10.next();
               BlockPos position = (new BlockPos(vec3d)).func_177963_a(vector.field_72450_a, vector.field_72448_b, vector.field_72449_c);
               switch(BlockUtil.isPositionPlaceable(position, raytrace)) {
               case -1:
               case 1:
   ...
   public static boolean isntValid(Entity entity, double range) {
      return entity == null || isDead(entity) || entity.equals(mc.field_71439_g) || entity instanceof EntityPlayer && esohack.friendManager.isFriend(entity.func_70005_c_()) || mc.field_71439_g.func_70068_e(entity) > MathUtil.square(range);
   }
   ...
   public static boolean isFakePlayer(EntityPlayer player) {
      ...
      if (freecam.isOn() && playerID == 69420) {
         ...
         if (fakePlayer.isOn()) {
            Iterator var4 = fakePlayer.fakePlayerIdList.iterator();
   ...
   public static Map<String, Integer> getTextRadarPlayers() {
      ...
      while(true) {
         ...
         if (!player.func_70005_c_().equals(mc.field_71439_g.func_70005_c_())) {
            ...
            ((Map)output).put(healthSB.toString() + " " + (esohack.friendManager.isFriend(player) ? "§b" : "§r") + player.func_70005_c_() + " " + distanceSB.toString() + " " + "§f" + esohack.totemPopManager.getTotemPopString(player) + esohack.potionManager.getTextRadarPotion(player), (int)mc.field_71439_g.func_70032_d(player));
