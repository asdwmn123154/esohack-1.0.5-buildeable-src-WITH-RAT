...
public class AutoArmor extends Module {
   ...
   public void onKeyInput(KeyInputEvent event) {
      if (Keyboard.getEventKeyState() && !(mc.field_71462_r instanceof esohackGui) && ((Bind)this.elytraBind.getValue()).getKey() == Keyboard.getEventKey()) {
      ...
      if (!fullNullCheck() && (!(mc.field_71462_r instanceof GuiContainer) || mc.field_71462_r instanceof GuiInventory)) {
         if (this.taskList.isEmpty()) {
            ...
            if (helm.func_77973_b() == Items.field_190931_a) {
               slot = InventoryUtil.findArmorSlot(EntityEquipmentSlot.HEAD, (Boolean)this.curse.getValue());
               if (slot != -1) {
            ...
            if (chest.func_77973_b() == Items.field_190931_a) {
               if (this.taskList.isEmpty()) {
                  if (this.elytraOn && this.elytraTimer.passedMs(500L)) {
                     slot = InventoryUtil.findItemInventorySlot(Items.field_185160_cR, false);
                     if (slot != -1) {
                     ...
                     slot = InventoryUtil.findArmorSlot(EntityEquipmentSlot.CHEST, (Boolean)this.curse.getValue());
                     if (slot != -1) {
               ...
               slot = InventoryUtil.findItemInventorySlot(Items.field_151163_ad, false);
               if (slot == -1) {
                  slot = InventoryUtil.findItemInventorySlot(Items.field_151030_Z, false);
                  if (slot == -1) {
                     slot = InventoryUtil.findItemInventorySlot(Items.field_151171_ah, false);
                     if (slot == -1) {
                        slot = InventoryUtil.findItemInventorySlot(Items.field_151023_V, false);
                        if (slot == -1) {
                           slot = InventoryUtil.findItemInventorySlot(Items.field_151027_R, false);
                        }
                     }
                  }
               }
            ...
            if (legging.func_77973_b() == Items.field_190931_a) {
               slot = InventoryUtil.findArmorSlot(EntityEquipmentSlot.LEGS, (Boolean)this.curse.getValue());
               if (slot != -1) {
            ...
            if (feet.func_77973_b() == Items.field_190931_a) {
               slot = InventoryUtil.findArmorSlot(EntityEquipmentSlot.FEET, (Boolean)this.curse.getValue());
               if (slot != -1) {
         ...
         if (this.timer.passedMs((long)((int)((float)(Integer)this.delay.getValue() * ((Boolean)this.tps.getValue() ? esohack.serverManager.getTpsFactor() : 1.0F))))) {
            if (!this.taskList.isEmpty()) {
   ...
   private void takeOffSlot(int slot) {
      if (this.taskList.isEmpty()) {
         int target = -1;
         Iterator var3 = InventoryUtil.findEmptySlots(false).iterator();
