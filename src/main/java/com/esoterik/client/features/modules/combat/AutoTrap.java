...
public class AutoTrap extends Module {
   ...
   private void doStaticTrap() {
      List<Vec3d> placeTargets = EntityUtil.targets(this.target.func_174791_d(), (Boolean)this.antiScaffold.getValue(), (Boolean)this.antiStep.getValue(), false, false, false, (Boolean)this.raytrace.getValue(), true);
   ...
   private EntityPlayer getTarget(double range, boolean trapped) {
      ...
      while(true) {
         ...
         } while(trapped && EntityUtil.isTrapped(player, (Boolean)this.antiScaffold.getValue(), (Boolean)this.antiStep.getValue(), false, false, false, true));

         if (!(esohack.speedManager.getPlayerSpeed(player) > 10.0D)) {
            if (target == null) {
               target = player;
