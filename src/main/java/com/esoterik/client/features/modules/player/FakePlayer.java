...
public class FakePlayer extends Module {
   private Setting<Boolean> copyInv = this.register(new Setting("CopyInv", true));
   public Setting<Boolean> multi = this.register(new Setting("Multi", false));
   private Setting<Integer> players = this.register(new Setting("Players", 1, 1, 9, (v) -> {
      return (Boolean)this.multi.getValue();
      ...
      if (fullNullCheck()) {
         ...
         if ((Boolean)this.multi.getValue()) {
            ...
            for(int var5 = 0; var5 < var4; ++var5) {
               ...
               ++amount;
               if (amount >= (Integer)this.players.getValue()) {
                  return;
               }
   ...
   private void addFakePlayer(String uuid, String name, int entityId, int offsetX, int offsetZ) {
      ...
      EntityOtherPlayerMP fakePlayer = new EntityOtherPlayerMP(mc.field_71441_e, profile);
      ...
      if ((Boolean)this.copyInv.getValue()) {
         Iterator var8 = esohack.potionManager.getOwnPotions().iterator();
         ...
         fakePlayer.field_71071_by.func_70455_b(mc.field_71439_g.field_71071_by);
      }

      fakePlayer.func_70606_j(mc.field_71439_g.func_110143_aJ() + mc.field_71439_g.func_110139_bj());
      this.fakeEntities.add(fakePlayer);
      mc.field_71441_e.func_73027_a(entityId, fakePlayer);
