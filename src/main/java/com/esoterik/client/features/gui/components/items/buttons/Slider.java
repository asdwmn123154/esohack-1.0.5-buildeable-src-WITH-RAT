...
public class Slider extends Button {
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks) {
      ...
      RenderUtil.drawRect(this.x, this.y, ((Number)this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float)this.width + 7.4F) * this.partialMultiplier(), this.y + (float)this.height - 0.5F, !this.isHovering(mouseX, mouseY) ? esohack.colorManager.getColorWithAlpha((Integer)((ClickGui)esohack.moduleManager.getModuleByClass(ClickGui.class)).hoverAlpha.getValue()) : esohack.colorManager.getColorWithAlpha((Integer)((ClickGui)esohack.moduleManager.getModuleByClass(ClickGui.class)).alpha.getValue()));
      esohack.textManager.drawStringWithShadow(this.getName() + " " + "§7" + (this.setting.getValue() instanceof Float ? (Number)this.setting.getValue() : ((Number)this.setting.getValue()).doubleValue()), this.x + 2.3F, this.y - 1.7F - (float)esohackGui.getClickGui().getTextOffset(), -1);
   }
